...
export interface GeminiAnalysisResult {
  ...
  dependencyGrade: string;
  testCoverageGrade: string;
 ...
 */
function runOfflineEvaluation(localData: LocalAnalysisResult): GeminiAnalysisResult {
  // 1. Calculate Code Quality Score
  let codeQualityScore = 100;
  if (localData.testFrameworks.length === 0 && !localData.testDirsDetected) codeQualityScore -= 20;
  if (!localData.hasApiDocs) codeQualityScore -= 10;
  if (!localData.hasReadme) codeQualityScore -= 10;
  ...
  if (codeQualityScore >= 80) codeQualityRationale = 'Good documentation and modular structure detected.';
  else if (codeQualityScore >= 50) codeQualityRationale = 'Basic project structure found, but lacks adequate testing or documentation.';
  else codeQualityRationale = 'Minimal project architecture with significant documentation or testing gaps.';

  // 2. Calculate Security Score & Map Issues
  let securityScore = 100;
  const securityIssues: GeminiAnalysisResult['securityIssues'] = [];
  ...
  localData.securityIssues.forEach(issue => {
    ...
    if (issue.severity === 'high') securityScore -= 25;
    else if (issue.severity === 'medium') securityScore -= 15;
  ...
  if (localData.hasReadme && localData.hasLicense && localData.hasApiDocs) documentationGrade = 'A';
  else if (localData.hasReadme && localData.hasLicense) documentationGrade = 'B';
  else if (localData.hasReadme) documentationGrade = 'C';
  ...
  if (npmDeprecated > 0) dependencyGrade = 'D';
  else if (npmOutdated > 5) dependencyGrade = 'C';
  else if (npmOutdated > 0) dependencyGrade = 'B';
  ...
  if (localData.hasReadme) keyStrengths.push('README file is present at the root directory.');
  if (localData.hasLicense) keyStrengths.push('Open-source LICENSE file is configured.');
  if (localData.testFrameworks.length > 0 || localData.testDirsDetected) keyStrengths.push('Testing framework or test directory detected.');
  if (keyStrengths.length === 0) keyStrengths.push('Basic repository file structure is established.');

  if (!localData.hasLicense) keyWeaknesses.push('No open-source LICENSE file detected.');
  if (localData.testFrameworks.length === 0 && !localData.testDirsDetected) keyWeaknesses.push('No tests or testing configurations found.');
  if (localData.securityIssues.length > 0) keyWeaknesses.push('Local secrets scanning detected potential hardcoded API keys or credentials.');
  if (keyWeaknesses.length === 0) keyWeaknesses.push('Minimal documentation or missing API references.');

  if (!localData.hasLicense) recommendations.push('Add an MIT or Apache 2.0 LICENSE file to clarify reuse guidelines.');
  if (localData.testFrameworks.length === 0) recommendations.push('Install Vitest or Jest and configure automated unit tests.');
  if (localData.securityIssues.length > 0) recommendations.push('Review the hardcoded credentials in the repository and move them to environment variables.');
