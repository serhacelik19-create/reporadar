...
import { analyzeDirectory, LocalAnalysisResult } from './analyzer.js';
import { analyzeWithGemini, GeminiAnalysisResult } from './gemini.js';
...
async function runAnalysis(repoUrl: string, options: AnalyzeOptions, isInteractiveLoop = false) {
 ...
 */
function evaluateQualityGates(aiData: GeminiAnalysisResult, options: AnalyzeOptions, isInteractiveLoop = false) {
  // Fail on Score gate
 ...
 */
async function runCompareFlow(repoUrl1: string, repoUrl2: string, apiKey?: string, isOffline = false) {
  const spinner = ora();
  let tempDir1 = '';
  let tempDir2 = '';
  ...
  try {
    ...
    printCompareReport(
      { owner: gitInfo1.owner, repo: gitInfo1.repo, local: localData1, ai: aiData1 },
      { owner: gitInfo2.owner, repo: gitInfo2.repo, local: localData2, ai: aiData2 }
    );
  ...
  while (keepLooping) {
    ...
    if (action === 'analyze-ai' || action === 'analyze-offline') {
      ...
      const questions: prompts.PromptObject[] = [
        ...
        {
          type: (prev: any) => (prev ? 'number' : null),
          name: 'failOnScore',
          message: 'Fail if quality score is below (0-100):',
          initial: 80,
          validate: (val: number) => (val >= 0 && val <= 100) || 'Score must be between 0 and 100.'
        },
        {
          type: 'confirm',
      ...
      const response = await prompts(questions, { onCancel });
      if (!response.repoUrl1 || !response.repoUrl2) continue;
      ...
      const configQuestions: prompts.PromptObject[] = [
        {
          type: 'password',
          name: 'apiKey',
      ...
      if (response.apiKey === undefined || response.model === undefined) continue;
