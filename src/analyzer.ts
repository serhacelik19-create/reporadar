 ...
 */
export async function analyzeDirectory(dirPath: string): Promise<LocalAnalysisResult> {
  ...
  function scan(currentDir: string, depth: number = 0, maxDepth: number = 4) {
          ...
          try {
            ...
            for (const line of lines) {
              ...
              if (clean && !clean.startsWith('#')) {
                ...
                if (match) result.pythonDependencies.push(clean);
              }
            }
          } catch {}
        }
  ...
  if (fs.existsSync(packageJsonPath)) {
    try {
      ...
      const depList = Object.entries(deps as Record<string, string>).slice(0, 12);
      if (depList.length > 0) {
 ...
 */
async function fetchDependenciesStatus(deps: Array<[string, string]>): Promise<DependencyStatus[]> {
